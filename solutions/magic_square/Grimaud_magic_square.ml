let load_csv filename =
  try
    ...
    let endOfColumn = ',' in
    let rec read_square square =
      try
        let line = input_line ic in
        let line_list = String.split_on_char endOfColumn line in
        let number_list = List.map (fun elem -> int_of_string (String.trim elem)) line_list in
        read_square square@[number_list]
      with
      | End_of_file -> square
    in
    let square = Array.of_list (List.map Array.of_list (read_square [])) in
    close_in ic; square
  with
